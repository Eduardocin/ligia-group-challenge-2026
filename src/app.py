...
CATEGORY_MAPS = {
    ...
    "exerciseangia": {"input": {"Sim": 1, "Não": 0}, "output": {0: "Não", 1: "Sim"}},
    "chestpain": {
        "input": {
            "Angina Típica": 0,
            "Angina Atípica": 1,
    ...
    "age_group": {"output": {0: "<40 anos", 1: "40-60 anos", 2: ">60 anos"}},
    "chol_category": {"output": {0: "Normal (<200)", 1: "Limítrofe (200-240)", 2: "Alto (>240)"}},
    "bp_category": {"output": {0: "Normal (<120)", 1: "Elevada (120-160)", 2: "Alta (>160)"}},
}
...
def make_prediction(model, scaler, data: dict) -> tuple:
    ...
    safe_data["chol_category"] = (
        0
        if safe_data["serumcholestrol"] <= 200
        else 1
        if safe_data["serumcholestrol"] <= 240
        else 2
    )
    safe_data["bp_category"] = (
        0 if safe_data["restingBP"] <= 120 else 1 if safe_data["restingBP"] <= 160 else 2
    )
...
def extract_features_from_pdfs(uploaded_files):
    ...
    if re.search(r"Angina durante esforço:.*?\[X]\s*Sim", full_text, re.DOTALL):
        data["exerciseangia"] = "Sim"
    elif re.search(r"Angina durante esforço:.*?\[X]\s*Não", full_text, re.DOTALL):
        data["exerciseangia"] = "Não"
    else:
        data["exerciseangia"] = None
...
def render_form(prefix):
    ...
    with col1:
        st.number_input(
            get_label("Idade", k("age")),
            min_value=18,
            max_value=100,
            key=k("age"),
            value=st.session_state[k("age")],
            placeholder="Digite...",
        )
        ...
        st.number_input(
            get_label("Pressão Arterial", k("restingBP")),
            80,
            250,
            key=k("restingBP"),
            value=st.session_state[k("restingBP")],
            placeholder="Ex: 120",
        )

        st.number_input(
            get_label("Colesterol (mg/dL)", k("serumcholestrol")),
            100,
            600,
            key=k("serumcholestrol"),
            value=st.session_state[k("serumcholestrol")],
            placeholder="Ex: 200",
        )

        st.number_input(
            get_label("Glicemia Jejum (mg/dL)", k("fastingbloodsugar")),
            0,
            500,
            key=k("fastingbloodsugar"),
            value=st.session_state[k("fastingbloodsugar")],
            placeholder="Ex: 90",
        )

    with col2:
        st.number_input(
            get_label("Frequência Cardíaca Máx", k("maxheartrate")),
            60,
            220,
            key=k("maxheartrate"),
            value=st.session_state[k("maxheartrate")],
            placeholder="Ex: 150",
        )

        st.number_input(
            get_label("Depressão ST (oldpeak)", k("oldpeak")),
            0.0,
            10.0,
            step=0.1,
            key=k("oldpeak"),
            value=st.session_state[k("oldpeak")],
            placeholder="Ex: 1.0",
        )

        curr_ves = st.session_state.get(k("noofmajorvessels"))
        st.selectbox(
            get_label("Nº Vasos Principais", k("noofmajorvessels")),
            [0, 1, 2, 3],
            index=curr_ves,
            key=k("noofmajorvessels"),
            placeholder="Selecione...",
        )
        ...
        st.selectbox(
            get_label("Angina por Exercício", k("exerciseangia")),
            opts_ang,
            index=idx_ang,
            key=k("exerciseangia"),
            placeholder="Selecione...",
    ...
    with col3:
        ...
        st.selectbox(
            get_label("Tipo de Dor no Peito", k("chestpain")),
            opts_cp,
            index=idx_cp,
            key=k("chestpain"),
            placeholder="Selecione...",
        ...
        st.selectbox(
            get_label("ECG em Repouso", k("restingrelectro")),
            opts_ecg,
            index=idx_ecg,
            key=k("restingrelectro"),
            placeholder="Selecione...",
        )
    ...
    with col4:
        ...
        st.selectbox(
            get_label("Inclinação ST (Slope)", k("slope")),
            opts_slp,
            index=idx_slp,
            key=k("slope"),
            placeholder="Selecione...",
...
def main():
    ...
    mode = st.radio(
        "Selecione o modo:",
        ["📝 Inserção Manual", "📂 Upload de Exames (PDF)"],
        index=0 if st.session_state.view_mode == "📝 Inserção Manual" else 1,
        horizontal=True,
    )
            ...
            with col_b1:
                if st.button("🔍 Analisar Dados PDF", type="primary"):
                    if any(v is None for v in data_pdf.values()):
                        ...
                        try:
                            res, X_data, features, shap_vals = make_prediction(
                                model, scaler, data_pdf
                            )
                            show_result(res, X_data, features, shap_vals, scaler)
                        except Exception as e:
                            st.error(f"Erro: {e}")
...
def show_result(prediction, X_data=None, feature_names=None, shap_values_local=None, scaler=None):
    ...
    if X_data is not None and feature_names is not None and shap_values_local is not None:
        ...
        st.caption(
            "Cada análise pode ter influenciadores DIFERENTES, dependendo dos valores do paciente"
        )
